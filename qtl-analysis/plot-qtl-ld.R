...
D = data.frame(CHR=rep(r[1], N), BEG=L[,2]-1, END=L[,2], R2=cor(t(M), M[I[r],]), stringsAsFactors=FALSE)
d = range(which(D$R2 >= min.ld))
d[1] = max(1, d[1] - 1)
d[2] = min(N, d[2] + 1)

write.table(D, sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE, file=sprintf("%s.R2.bed",o))
cat(sprintf("%s\t%d\t%d\n", l[1], D$BEG[d[1]], D$END[d[2]]), file=sprintf("%s.%d.bed",o,as.integer(100*min.ld)))

pdf(sprintf("%s.pdf",o))
par(mar=c(5.1,6.1,4.1,2.1))
plot(D$END/1e6, D$R2, type='o', 
